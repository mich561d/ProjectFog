...
public class BasePoleCalculator {
    ...
    private void AddPartToList(int height, int poles, ArrayList<Part> parts) throws FogException {
        int calcHeight = height + Rules.POLELENGTHUNDERGROUND;
    ...
    private int calcPolesSides(int length) {
        int poles = 0;
        double estPolesSides = ((double) length - 30) / (double) Rules.MAXDISTANCEBEWTEENPOLES;
    ...
    private int calcPolesBack(int width) {
        int poles = 0;
        if (width >= Rules.MAXDISTANCEBEWTEENPOLES) {
            double estPolesSides = ((double) width - 30) / (double) Rules.MAXDISTANCEBEWTEENPOLES;
            poles += Math.ceil(estPolesSides) - 1;
        }
        return poles;
    }
